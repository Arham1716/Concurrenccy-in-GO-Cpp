...
import cpuGraphCPP from '../assets/cpp_graphs/ProducerConsumer_CPU.png';
import memoryGraphCPP from '../assets/cpp_graphs/ProducerConsumer_Memory.png';
import cpuGraphGO from '../assets/go_graphs/ProducerConsumer_CPU.png';
import memoryGraphGO from '../assets/go_graphs/ProducerConsumer_Memory.png';
  ...
  const runSimulation = async () => {
    const apiUrl =
      selectedLanguage === "cpp"
        ? "http://localhost:5000/ProducerConsumer"
        : "http://localhost:5001/ProducerConsumer";
    ...
    <>
      <div className="container d-flex flex-column align-items-center py-5">
        ...
        <div className="problem-container p-4 text-center">
          <h3 className="problem-title">Problem Title: Producer Consumer</h3>

          {/* Problem Description */}
          <div className="problem-description text-start">
            <p>
              The <strong>Producer-Consumer Problem</strong> is a classic synchronization
              problem where producers add data to a shared buffer and consumers remove
              data from the buffer. The buffer has a fixed size, so producers must
              wait if the buffer is full, and consumers must wait if the buffer is empty.
            </p>
            <p>
              The challenge is to ensure proper synchronization between producers and
              consumers, prevent race conditions when accessing the shared buffer,
              and avoid deadlocks while maintaining efficient operation.
            </p>
          </div>
